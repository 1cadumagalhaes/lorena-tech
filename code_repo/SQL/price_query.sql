WITH getMinMax AS (WITH D AS (WITH geo AS (SELECT 
  *,
...
ORDER BY
  distance ASC)
SELECT MAX(minimum_estimate) as maximum_value, MIN(minimum_estimate) as minimum_value, COUNT(minimum_estimate) as number FROM getMinMax 